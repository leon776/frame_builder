...
var App = function(cfg) {
	...
	*/
	...
	var cfg = cfg,
		...
		Page = [],
		curPage = cfg.startPage || 0,//当前所在的页面
		tmpText,
		tmpImg,
		tmpImg2,
		fps = 1000 / cfg.fps;
	...
	var _app = {
		...
		*/
        ...
        _color2img: function(color) {
            var _cvs = document.createElement('canvas');
            var _ctx = _cvs.getContext("2d");
            _ctx.fillStyle = color;
            _ctx.fillRect(0, 0, cvs.width, cvs.height);
            return _cvs.toDataURL();
        },
		/*
			# 根据配置文件创建页面对象
			# return 
		...
		*/
		createPage: function() {
			...
			for (var i = 0; i < cfg.pageNum; i++) {
				tmp = {
					backgroundimg: cfg.page[i].backgroundtype !== -1 ? 'img/page' + (i + 1) + '/' + cfg.page[i].backgroundimg : this._color2img(cfg.page[i].backgroundimg),
